...
|--------|---------------------|----------|------|
| **Detailed** | ~More than 300k | Analysis, joins, filtering | Large |
...
```
Input: 300k rows
Detailed output: 300k rows (same)
Condensed output: ~87 rows (4,217x reduction! 🎉)
```
...
### Generated Files
```
output_detailed.csv   (300k rows)
output_condensed.csv  (~87 rows)
output_summary.csv    (~87 rows)
```
...
|--------|-----------|------|-------------------|
| Input | ~15 MB | 300k | 30-45 seconds |
| Detailed | ~18 MB | 300k | 30-45 seconds |
| **Condensed** | **~25 KB** | **~87** | **<1 second** ⚡ |
| Summary | ~30 KB | ~87 | <1 second ⚡ |
...
**Input**: 300k security-related items with typos
...
For your 300k row dataset, **CONDENSED is highly recommended** to reduce from 300k rows to ~87 rows! 🎉